      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Unidad_medida</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Unidad_medida</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Unidad_medida</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar unidad_medida':'Añadir unidad_medida'"
        class="modal-form"
        width="55rem"
      ...
      >
        <unidad_medida_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_um"
        :dataSource="data"
...
import unidad_medida_form from "../form/unidad_medida_form";
...
export default {
  ...
  data() {
    return {
      ...
      unidad_medida_list: [],
      filter: null,
      columns: mb.statics('Unidad_medida').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Unidad_medida'),
  ...
  watch: {
    filter: function() {
      this.data = this.unidad_medida_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Unidad_medida');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Unidad_medida').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Unidad_medida').list(params);
        this.unidad_medida_list = resp;
        this.data = this.unidad_medida_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Unidad_medida',model);
      this.showModalForm();
    }
  },
