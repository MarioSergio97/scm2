      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Incidencia</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <!--<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">-->
              ...
              <!--</li>-->
              <!--<li class="breadcrumb-item">-->
                <!--<a href="" class="text-muted">Incidencia</a>-->
              <!--</li>-->
            <!--</ul>-->
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de Incidencias</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar incidencia':'Añadir incidencia'"
        class="modal-form"
        width="55rem"
      ...
      >
        <incidencia_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_incidencia"
        :dataSource="data"
...
export default {
  ...
  data() {
    return {
      ...
      selected_model: mb.instance( 'Incidencia'),
  ...
  watch: {
    filter: function() {
      this.data = this.incidencia_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Incidencia');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Incidencia').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Incidencia').list(params);
        this.incidencia_list = resp;
        this.data = this.incidencia_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Incidencia',model);
      this.showModalForm();
    }
  },
