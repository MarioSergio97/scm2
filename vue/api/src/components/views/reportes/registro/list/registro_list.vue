      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Registro</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Registro de operaciones</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar registro':'Añadir registro'"
        class="modal-form"
        width="55rem"
...
export default {
  ...
  data() {
    return {
      ...
      registro_list: [],
      filter: null,
      columns: mb.statics('Registro').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Registro'),
  ...
  watch: {
    filter: function() {
      this.data = this.registro_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Registro');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Registro').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['usuario','entidad','proceso'];

        const resp = await mb.statics('Registro').list(params);
        this.registro_list = resp;
        this.data = this.registro_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Registro',model);
      this.showModalForm();
    }
  },
