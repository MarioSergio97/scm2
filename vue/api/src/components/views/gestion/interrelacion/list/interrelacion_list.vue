      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Interrelación</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <!--<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">-->
              ...
              <!--</li>-->
              <!--<li class="breadcrumb-item">-->
                <!--<a href="" class="text-muted">Interrelación</a>-->
              <!--</li>-->
            <!--</ul>-->
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de Interrelaciones</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar interrelacion':'Añadir interrelacion'"
        class="modal-form"
        width="55rem"
      ...
      >
        <interrelacion_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_interrelacion"
        :dataSource="data"
...
import interrelacion_form from "../form/interrelacion_form";
...
export default {
  ...
  data() {
    return {
      ...
      selected_model: mb.instance( 'Interrelacion'),
  ...
  watch: {
    filter: function() {
      this.data = this.interrelacion_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Interrelacion');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Interrelacion').delete_by_ids(
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Interrelacion').list(params);
        this.interrelacion_list = resp;
        this.data = this.interrelacion_list.data.filter(this.filter_data);
        var interrelacion=[];

        for(var i=0; i<this.data.length; i++){
            interrelacion[i] = mb.instance('Interrelacion',this.data[i]);
        }
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Interrelacion',model);
      this.showModalForm();
    }
  },
