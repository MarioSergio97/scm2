      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Parametro_capacidad</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
              ...
              <li class="breadcrumb-item">
                <a href="" class="text-muted">Parametro_capacidad</a>
              </li>
            </ul>
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Parametro_capacidad</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar parametro_capacidad':'Añadir parametro_capacidad'"
        class="modal-form"
        width="55rem"
      ...
      >
        <parametro_capacidad_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_parametro"
        :dataSource="data"
...
import parametro_capacidad_form from "../form/parametro_capacidad_form";
...
export default {
  ...
  data() {
    return {
      data: [],
      self: null,
      parametro_capacidad_list: [],
      filter: null,
      columns: mb.statics('Parametro_capacidad').columns,
      loading: false,
  ...
  components: {
    ...
    vantdpagination,
    parametro_capacidad_form
  },
  watch: {
    filter: function() {
      this.data = this.parametro_capacidad_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Parametro_capacidad');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Parametro_capacidad').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['capacidad'];

        const resp = await mb.statics('Parametro_capacidad').list(params);
        this.parametro_capacidad_list = resp;
        this.data = this.parametro_capacidad_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Parametro_capacidad',model);
      this.showModalForm();
    }
  },
...
<style>
@import "parametro_capacidad_list.css";
</style>
