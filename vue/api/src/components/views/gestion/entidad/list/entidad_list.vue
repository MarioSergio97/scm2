      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Entidad</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <!--<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">-->
              ...
              <!--</li>-->
             <!--&lt;!&ndash;  <li class="breadcrumb-item">-->
                <!--<a href="/" class="text-muted">Gestión</a>-->
              <!--</li> &ndash;&gt;-->
              <!--<li class="breadcrumb-item">-->
                <!--<a href="" class="text-muted">Entidad</a>-->
              <!--</li>-->
            <!--</ul>-->
          </div>
        </div>
...
<!--          </div>-->
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar entidad':'Añadir entidad'"
        class="modal-form"
        width="55rem"
      ...
      >
        <entidad_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_entidad"
        :dataSource="data"
...
import entidad_form from "../form/entidad_form";
...
export default {
  ...
  data() {
    return {
      ...
      entidad_list: [],
      filter: null,
      columns: mb.statics('Entidad').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Entidad'),
  ...
  watch: {
    filter: function() {
      this.data = this.entidad_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    onCloseModal(e,reload_data=false) {
      this.selected_model = mb.instance('Entidad');
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Entidad').delete_by_ids(
    ...
    async load_data() {
      try {
        this.loading = true;
        var params = {};
        params.relations=['pais','provincia'];

        const resp = await mb.statics('Entidad').list(params);
        this.entidad_list = resp;
        this.data = this.entidad_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
    ...
    onEditing(model) {
      this.selected_model = mb.instance('Entidad',model);
      this.showModalForm();
    }
  },
