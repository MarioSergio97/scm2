...
<div class="subheader py-2 py-lg-6 subheader-solid" id="kt_subheader">
      ...
      <div class="row">
        <div class="col-md-6">
          <a-button-group style="margin-bottom: 10px">
            ...
            <a-tooltip placement="topLeft" title="Mostar capacidad del proceso seleccionado">
              <a-button icon="plus" type="primary" @click="showCapacidadProceso">Abrir</a-button>
            </a-tooltip>
   ...
   <div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar capacidad':'Añadir capacidad'"
        class="modal-form"
        width="55rem"
...
import capacidad_form from "../form/capacidad_form";
...
export default {
  ...
  data() {
    return {
      ...
      capacidad_list: [],
      filter: null,
      columns: mb.statics('Capacidad').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Capacidad'),
      show_modal_form: false,
      show_capacidad_proceso: false,
      mb,
      prueba:'',
    };
  },
  ...
  methods: {
    ...
    onChange: function(selectedRowKeys) {
      ...
      eventBus.idCapacidadSelected = this.selectedRowKeys[0];
      this.prueba = eventBus.idCapacidadSelected;
    },
    showDeleteConfirm() {
    ...
    async load_data() {
      try {
        ...
        const resp = await mb.statics('Capacidad').list(params);
        this.capacidad_list = resp;
        this.data = this.capacidad_list.data.filter(this.filter_data);
        this.loading = false;
      } catch (error) {
        utils.process_error(error);
