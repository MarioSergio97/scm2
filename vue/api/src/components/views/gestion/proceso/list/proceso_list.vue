      ...
      <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
          ...
          <div class="d-flex align-items-baseline flex-wrap mr-5">
            <!--begin::Page Title-->
            <h5 class="text-dark font-weight-bold my-1 mr-5">Proceso</h5>
            <!--end::Page Title-->
            <!--begin::Breadcrumb-->
            <!--<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">-->
              ...
              <!--</li>-->
              <!--<li class="breadcrumb-item">-->
                <!--<a href="" class="text-muted">Proceso: vista ampliada</a>-->
              <!--</li>-->
            <!--</ul>-->
          </div>
        </div>
   ...
   <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de  Proceso</h6>
          </div>
      ...
      <a-modal
        @cancel="onCloseModal"
        :title="selected_model.get_id()?'Actualizar proceso':'Añadir proceso'"
        class="modal-form"
        width="55rem"
      ...
      >
        <proceso_form  :modal=true :model="selected_model" />
      </a-modal>

      <a-table
        :columns="columns"
        :rowKey="record => record.id_proceso"
        :dataSource="data"
...
import proceso_form from "../form/proceso_form";
...
export default {
  ...
  data() {
    return {
      ...
      proceso_list: [],
      filter: null,
      columns: mb.statics('Proceso').columns,
      loading: false,
      text_select: "Select All",
      selectedRowKeys: [],
      pagination: vantdpagination,
      selected_model: mb.instance( 'Proceso'),
  ...
  watch: {
    filter: function() {
      this.data = this.proceso_list.data.filter(this.filter_data);
    },
  ...
  methods: {
    ...
    showDeleteConfirm() {
      ...
      this.$confirm({
        ...
        async onOk() {
          try {
            const response = await mb.statics('Proceso').delete_by_ids(
                _this.selectedRowKeys
              );
               utils.process_response(response, "eliminar");
              _this.selectedRowKeys=[];
              _this.load_data();
            } catch (error) {
    ...
    onEditing(model) {
      ...
      if(eventBus.user.id_entidad == this.selected_model.id_entidad || eventBus.esGestor){
        ...
        utils.openNotificationWithIcon(
                        "error",
                        "Modificar proceso.",
                        "Usted no puede modificar este proceso."
                    );
      }
