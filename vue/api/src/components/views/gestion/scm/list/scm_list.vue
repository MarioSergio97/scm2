  ...
  <div>
    ...
    <div class="container">
      <div>
        ...
        <!--        <p>id de la entidad del usuario = {{user.id_entidad}}</p>-->
    ...
    <div>
      <div class="card card-custom">
        <div class="card-header py-3">
          <div class="col-md-6">
            <h6 style="text-align: left">Listado de Cadenas de Suministro</h6>
          </div>
    ...
    <a-modal
      @cancel="onCloseModal"
      :title="selected_model.get_id()?'Actualizar cadena de suministro':'Añadir cadena de suministro'"
      class="modal-form"
      width="55rem"
...
export default {
    ...
    selectSCM_action() {
      if (this.selectedRowKeys.length == 0) {
        utils.openNotificationWithIcon(
          "error",
          "Acceder a la cadena de suministro.",
    ...
    async buscarEntidadGestora() {
      try {
        ...
        if (scmSer.length > 0) {
          ...
          console.log('mi id de gestora func buscar entidad gest es -> ', eventBus.idEntidadGestora);
        }
    ...
    onEditing(model) {
      ...
      if (eventBus.esAdmin) {
        ...
        utils.openNotificationWithIcon(
          "error",
          "Modificar cadena.",
          "Usted no puede modificar esta cadena."
        );
      }
    }
  },
  ...
  mounted() {
    ...
    this.idGestora = eventBus.idEntidadGestora;
    this.loading = true;
    this.scm_selected = false;
    this.data = [];
    this.load_data();
