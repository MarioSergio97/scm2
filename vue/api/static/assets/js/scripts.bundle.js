...
(function($) {
		...
		var Extension = {
			...
			init: function() {
				if (Extension.selectorEnabled()) {
					...
					// Seleccionar Todo on extension init
			 ...
			 */
			initEvent: function() {
				// Seleccionar Todo checkbox click
				$(datatable.tableHead).find('.' + pfx + 'checkbox-all > [type="checkbox"]').click(function(e) {
					// clear selected and unselected rows
					Extension.selectedRows = Extension.unselectedRows = [];
					datatable.stateRemove('checkbox');

					// Seleccionar Todo rows
					Extension.selectedAllRows = !!$(this).is(':checked');

					// local Seleccionar Todo current page rows
					if (!options.vars.requestIds) {
						if ($(this).is(':checked')) {
				...
				$(datatable.tableBody).find('.' + pfx + 'checkbox-single > [type="checkbox"]').click(function(e) {
					...
					if (!options.vars.requestIds && Extension.selectedRows.length < 1) {
						// remove Seleccionar Todo checkbox, if there is no checked checkbox left
			...
			initSelect: function() {
				...
				if (Extension.selectedAllRows && options.vars.requestIds) {
					if (!datatable.hasClass(pfx + 'datatable-error')) {
						// set header Seleccionar Todo checkbox checked
	...
	$.fn[pluginName].checkbox.default = {
		vars: {
			// Seleccionar Todo rows flag to be sent to the server
